	...
	<view class="border-top border-light-secondary p-3">
		...
		<view class="flex flex-wrap" style="margin-left: -20rpx;margin-right: -20rpx;">
			<view style="width: 33.3%; box-sizing: border-box;" class="p-2" v-for="(item,index) in list" :key="index" @click="chooseCoin(index)">
				<view v-if="item.price > 0" style="height: 130rpx;" class="border rounded flex flex-column align-center justify-center" :class="activeIndex === index ? 'border-main' : '' ">
				...
				<uni-popup ref="popup" type="dialog">
					<uni-popup-dialog mode="input" message="自定义充值" :duration="2000" placeholder="充值金额"
					inputType="number" :before-close="true" 
