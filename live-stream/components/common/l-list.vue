<template>
	...
	<!-- <view class="flex flex-wrap"> -->
		<view class="list-item" @click="$emit('click')">
			<image :src="item.cover" style="width: 365rpx; height: 365rpx;" class="rounded" mode="aspectFill">
			</image>
			...
			<view class="rounded-circle px-2 flex align-center" style="position: absolute;right: 15rpx; bottom: 15rpx; background-color: rgba(0,0,0,0.4);" >
				<text style="width: 20rpx; height: 20rpx;" class="rounded-circle  mr-1"
				:class=" 'bg-' + (item.status === 1 ? 'success' : 'danger')"></text>
